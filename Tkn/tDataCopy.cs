    ...
    public class tDataCopy:tBase
    {
        ...
        public Boolean tDC_Run(Form tForm, string DC_Code)
        {
            ...
            if ((work_type == 3) && (v.con_DragDropEdit == false))
            {
                ...
                if ((target_cntrl.ToString() == "DevExpress.XtraGrid.Views.Grid.GridView") ||
                    (target_cntrl.ToString() == "DevExpress.XtraGrid.Views.BandedGrid.AdvBandedGridView") ||
                    (target_cntrl.ToString() == "DevExpress.XtraGrid.Views.Layout.LayoutView") ||
                    (target_cntrl.ToString() == "DevExpress.XtraVerticalGrid.VGridControl") ||
                    (target_cntrl.ToString() == "DevExpress.XtraTreeList.TreeList") ||
                    (target_cntrl.ToString() == "DevExpress.XtraDataLayout.DataLayoutControl")
                   )
                {
                     ...
                     string Target_TableIPCode, string Source_TableIPCode)
        {
            ...
            if (t.IsNotNull(table_name) && t.IsNotNull(where_and))
            {
                try
                {
                    ...
                    ev.btn_Navigotor_Click(tForm, null, Source_TableIPCode, "listele", "", "", "12");
                ...
                string source_checkfname, string source_checkvalue)
        {
            ...
            if (row_type == 1)
            {
                ...
                if (Fill_Line(tForm, ds_Target, ds_Source, ds_DCLine, 
                              work_type, Target_TableIPCode, Source_TableIPCode, -1) == false)
                {
                    onay = false;
                    return onay;
                }
                ...
                (row_type == 3))
            {
                ...
                for (int iSourcePos = 0; iSourcePos < iSourceCount; iSourcePos++)
                {
                    ...
                    if (lkp_onay)
                    {
                        ...
                        if (Fill_Line(tForm, ds_Target, ds_Source, ds_DCLine, 
                                      work_type, Target_TableIPCode, Source_TableIPCode, iSourcePos) == false)
                        {
                            onay = false;
                            break;
                        }
                                  ...
                                  int Source_Position)
        {
            ...
            for (int i = 0; i < i1; i++)
            {
                ...
                if (dc_source_type == 3)
                {
                    ...
                    int ftype = t.Find_Field_Type_Id(ds_Target, target_fieldname, ref displayFormat);
                ...
                && (onay))
            {
                ...
                //tEvents ev = new tEvents();
                //v.btn_Navigotor_Click(tForm, null, Target_TableIPCode, "kaydet", "Kaydet", "", "24");
