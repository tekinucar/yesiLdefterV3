...
*/
            ...
            // 1. NOTE(@Janberk): InitPreparingConnection() sets up SQL connection strings for Manager DB, UstadCRM DB, and master DB.
            ...
            // 2. NOTE(@Janberk): InitLoginUser() opens the ms_User login form (Dialog mode). This is where authentication happens.
            // After successful login, v.SP_UserLOGIN becomes true and control returns here to continue initialization.
            t.WaitFormOpen(v.mainForm, "Kullunacı girişi...");
            if (v.active_DB.localDbUses == false)
            {
                System.Diagnostics.Debug.WriteLine("tStarter.InitStart → InitLoginUser (opening ms_User form)");
                InitLoginUser(); // Ustad YesiLdester user girişi
                System.Diagnostics.Debug.WriteLine($"tStarter.InitStart ← InitLoginUser (v.SP_UserLOGIN={v.SP_UserLOGIN})");
            }
            else
            {
            ...
            // NOTE(@Janberk): v.SP_UserIN = true, initialization is complete.
            ...
            // - SYS_Glyph_Read()
            // - TestRead()
            System.Diagnostics.Debug.WriteLine("tStarter.InitStart → Setting v.SP_UserIN = true (ready for main form)");
            v.SP_UserIN = true;
            System.Diagnostics.Debug.WriteLine("tStarter.InitStart ← Initialization complete (v.SP_UserIN=true)");
        }
        ...
        // FormCode "UST/CRM/ABO/UstadUserLogin" is used by tLayout.Create_Layout() to load the form's layout metadata from MS_LAYOUT table.
        // After user authenticates, the form closes and v.SP_UserLOGIN is set to true.
        void InitLoginUser()
        {
            string FormName = "ms_User";
            string FormCode = "UST/CRM/ABO/UstadUserLogin";
            System.Diagnostics.Debug.WriteLine($"tStarter.InitLoginUser: Opening {FormName} with FormCode={FormCode}");
            OpenFormPreparing(FormName, FormCode, v.formType.Dialog);
            System.Diagnostics.Debug.WriteLine($"tStarter.InitLoginUser: Login form closed, v.SP_UserLOGIN={v.SP_UserLOGIN}");
        }
