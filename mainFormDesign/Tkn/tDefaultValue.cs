...
namespace Tkn_DefaultValue
{
    public class tDefaultValue:tBase
    {
        ...
        public Boolean tDefaultValue_And_Validation(
                               Form tForm,
                               ...
                               string Kim)
        {
            ...
            foreach (DataRow FieldRow in dsMS_Fields_IP.Tables[0].Rows)
            {
                    ...
                    )
                {
                    ...
                    if (dfl_type != 0)
                    {
                        default_fill_err = default_fill_err +
                ...
                if (vld_operator > 11)
                {
                    ...
                    if ((Kim == "tData_Save") ||
                        ...
                        //(Kim == "tData_NewRecord"))
                    {
                        checked_sonucu = tValidation_Check(FieldRow, focus_data_row);

                        if (checked_sonucu != "ROW_DELETE")
                            validation_err = validation_err + checked_sonucu;
                        else
                        {
                            validation_err = checked_sonucu; // ROW_DELETE mesajı gidecek
                            break;
                        }   
                    }
        ...
        private string tDefault_Value_Fill(Form tForm,
                                           DataRow Field_Row,
                                           ...
                                           string Kim)
        {
            ...
            if (fautoinc == "True") { return err; }

            try
            {
                // daha önce column doldurulmuş ise bir daha atama yapmasın
                if (t.IsNotNull(focus_row[fname].ToString())) { return err; }
            }
            catch (Exception e)
            {
                            ...
                            if (Kim == "tData_Save")
                            {
                                if (dsData_Source.Tables[0].Columns[0].Caption.ToString() == master_FieldName)
                                {
                                        ...
                                        (value.ToString() == ""))
                                    {
                                        ...
                                        tSave sv = new tSave();
                                        if (sv.tDataSave(tForm, master_TableIPCode))
                                        {
                ...
                (default_type == 24) || (default_type2 == 24) )
            {
                ...
                if (cntrl != null)
                {
                    ...
                    if (parent != null)
                    {
                        if (parent.Tag != null)
                        {
                            string TagValue = parent.Tag.ToString();
                            ...
                            if (t.IsNotNull(TagValue))
                            {
                                ...
                                if (TagValue.IndexOf("||") > -1)
                                {
                                    ...
                                    value = t.Get_And_Clear(ref TagValue, "||");
